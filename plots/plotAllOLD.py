...
pyplot.plot(time,ext[9:,0], label = r'$F_{ext}(x)$');
pyplot.plot(time,ext[9:,1], label = r'$F_{ext}(y)$');
pyplot.plot(time,ext[9:,2], label = r'$F_{ext}(z)$');
#pyplot.xticks([])
pyplot.xticks(np.arange(min(time), max(time)+1, 5))
